...
viewer = 'D1836' #for optimizing on downstream viewer
...
spaceArray = [ {'name': 'v13', 'type': 'continuous', 'domain': (-5, 5)},  {'name': 'v31', 'type': 'continuous', 'domain': (-5, 5)} ]
	...
	if sandbox != 'y':
		...
		if (count_iter>0 and pk_1 < init_peak*0.05):
			...
			if (count_iter>0 and pk_1 < init_peak*0.05):
				...
				sleep(5)
				q14_frac_im= SaveIm('Q1frac23', viewer)
				
				#take picture with q14 zero, and q15 0.437
				SetMagnet(quad_pair[0], -40)
				SetMagnet(quad_pair[1], 20)
				...
				if (count_iter>0 and pk_1 < init_peak*0.05):
					...
					sleep(5)
					q15_frac_im= SaveIm('Q1frac043', viewer)
